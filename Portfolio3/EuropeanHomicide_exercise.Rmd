...
```{r Graf-Of-Homicides-In-Europa, eval = TRUE}
...
Western_Europe %>% 
  ...
  facet_wrap(~ Country)
ggsave("homicedes.png", width = 16, height = 8, dpi = 150)
...
Both show a long‑run decline in lethal violence:
The homicide graphs display a very steep, drop from very high levels in the late Middle Ages/early modern period toward very low rates by the 19th–20th century.
The monarch graph instead shows a gradual increase in reign length over time, with a lot of scatter early on but a clear upward trend from under about 15 years on average before 1200 toward around 30 years or more by the 1800s.
Interpreting them together, longer and more stable reigns of Danish monarchs parallel the large, secular decline in homicide rates across Europe and Scandinavia, suggesting increasing political stability and state capacity accompanying the pacification of everyday life.


```{r Danish-Monarchs-Throgh-1900}
# YOUR CODE HERE:
kings <- read_csv2("data/danish_monarchs.csv")

kings%>%
  mutate(duration=ReignedUntil-ReignedFrom)%>%
  mutate(midyear=ReignedUntil-duration/2)%>%
  ggplot(aes(x=midyear, y=duration))+
  geom_point(color="blue")+
  geom_smooth()+
  theme_bw()+
  labs(title = "Danish Monarchs Throgh 1900", x = "Year", y = "Duration Of Reign")
...
The visualizations suggest that, we are more civilized today.
The homicide charts show a dramatic, centuries‑long decline in lethal violence across England, Italy, the Low Countries, Germany/Switzerland, and Scandinavia, from extremely high levels (often dozens of killings per 100,000 people) in the late medieval and early modern periods to very low rates in the 19th and 20th centuries. This pattern implies that interpersonal conflicts that once frequently ended in death are now far more often managed through non‑lethal means such as law, courts, and social norms against violence.
The Danish monarchy chart complements this picture by showing that reigns, which were short and highly variable around the year 1000, lengthen and stabilize over time, especially after 1500. Longer reigns suggest fewer violent successions, stronger institutions, and more predictable governance, all of which generally reduce incentives for private violence.
Taken together, the data depict a long‑term move toward more stable political orders and much lower tolerance for homicide, both of which fit a reasonable definition of os as people becoming “more civilized.”
