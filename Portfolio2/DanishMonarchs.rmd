...
kings3 <- read_csv2("data/danish_monarchs.csv")
...
Glimpse shows a compact, transposed view of my entire dataset structure Furthermore it shows how many rows and columns I have and each of the columns names and how R interprets each column (data type: <chr> for text, <dbl> for numbers, etc.) and a preview of how the dataset looks 
...
# YOUR CODE
kings <- kings %>%
  mutate(duration = as.numeric(ReignedUntil) - as.numeric(ReignedFrom))

mean(kings$duration, na.rm = TRUE)

# The average duration of reign in my data set is  20.96 years
```
...
# YOUR CODE
kings %>%
  select(KingOrQueen, duration)%>%
  filter(duration > 20.9)
```
...
kings %>%
  ...
  select(KingOrQueen, duration, days)
```
#The three longest-ruling monarchs in days is:
1. Christian 4.	with 60 years	that is 21960 days		
2. Margrethe 2. with 52 years that is	19032	days	
3. Erik af Pommern with	43 years that is 15738 days	
...
# YOUR CODE
kings %>%
  ggplot(aes(x = ReignedFrom, y = duration)) +
  geom_point() +
  geom_line() +
